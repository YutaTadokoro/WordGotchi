...
- Google Cloud Project ID management
- Adding authentication headers
- Building API endpoints

**Proxy Server Implementation Example:**

The proxy server must provide the following endpoints:
- `POST /generate` - Receives image generation requests and forwards them to Gemini Imagen API

Configure the following on the proxy server side:
1. Gemini API key
2. Google Cloud Project ID
3. Forward requests to `https://us-central1-aiplatform.googleapis.com/v1/projects/{PROJECT_ID}/locations/us-central1/publishers/google/models/imagen-3.0-generate-001:predict`

See `proxy-server-example.md` for detailed implementation examples.
...
Both APIs operate via proxy servers. This provides:
- No API key exposure in the frontend
- Avoids CORS issues
- Centralized authentication and request management

See `proxy-server-example.md` for proxy server implementation examples.
